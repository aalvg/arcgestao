...
class Produto extends Model
{
	protected $fillable = [
		...
		'valor_compra', 'gerenciar_estoque', 'estoque_minimo', 'referencia', 'tela_id', 
		'largura', 'comprimento', 'altura', 'peso_liquido', 'peso_bruto', 'pRedBC', 
		'cBenef', 'CEST', 'referencia_grade', 'str_grade', 'grade','perc_glp', 'perc_gnn', 
		'perc_gni',  'valor_partida', 'unidade_tributavel', 'quantidade_tributavel', 
		'referencia_balanca', 'passageiros', 'cor_veiculo', 'chassi', 'combustivel',
		'numero_motor', 'ano', 'modelo', 'perc_icms_interestadual', 'perc_icms_interno',
		'perc_fcp_interestadual', 'origem'
	];
	...
	public function valorMediaDeCompra(){
		$soma = 0;
		$cont = 0;

		if(sizeof($this->emCompras) == 0) return 0;
		foreach($this->emCompras as $e){
			$soma += $e->valor_unitario;
			$cont++;
		}
	...
	public function valorDaUltimaCompra(){
		if(sizeof($this->emCompras) == 0) return 0;
		return $this->emCompras[sizeof($this->emCompras)-1]->valor_unitario;
	}
