...
class PedidoPrint extends Common
{
    ...
    public function montaDANFE(
        ...
        $tamPapelVert = (100 + $qrCodeExiste) + 16 + (($qtdItens - 1) * $hMaxLinha+(20*$qtdItens)) + ($qtdPgto * $hLinha);
        ...
        $y = $this->pTotalDANFE($x, $y + ($this->totalItens)+(10*$qtdItens), $hTotal);
        // //COLOCA PAGAMENTOS
...
protected function pProdutosDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    $hBoxLinha = $this->hBoxLinha;
    $hMaxLinha = $this->hMaxLinha+12;
    $cont = 0;
    $aFontProdutos = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'b');
    if ($qtdItens > 0) {
...
protected function pTotalDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    $texto = "R$ " . $this->venda->valor_total;
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
...
/*     $yTotalFinal = $y + ($hLinha*9);
    ...
    if($this->venda->endereco != null){
        ...
        $this->venda->endereco->bairro() . " | " . $this->venda->endereco->referencia . " (" . $this->venda->telefone . ")";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
